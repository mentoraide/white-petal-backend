...
export const getAllLibraryRecycleItems = (req: Request, res: Response): void => {
  LibraryRecycleBinModel.find()
    .populate("uploadedBy", "name email")
    .then((items) => res.json(items))
    .catch((err) => res.status(500).json({ message: err.message }));
};

export const restoreLibraryVideo = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const permanentDeleteLibraryVideo = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  try {
